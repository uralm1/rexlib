...
<% for my $net (@$_client_nets) { %>
-A forwarding_rule -s <%= $net %> -j clients_out
-A forwarding_rule -d <%= $net %> -j clients_in
<% } %>
COMMIT
...
<% for my $net (@$_client_nets) { %>
-A FORWARD -s <%= $net %> -j pipe_out_inet_clients
-A FORWARD -d <%= $net %> -j pipe_in_inet_clients
<% } %>
COMMIT
